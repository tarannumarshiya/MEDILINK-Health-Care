...
export default function EmergencyLoginPage() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    const allowedSet = new Set(["EMERGENCY", "HOSPITAL_ADMIN", "SUPER_ADMIN"]);
    if (!profile || !allowedSet.has(profile.role) || !profile.is_active) {
      await supabase.auth.signOut();
      setError(!allowedSet.has(profile?.role ?? "") ? "Access denied. Emergency staff only." : "Account inactive. Contact Super Admin.");
      setLoading(false);
      return;
    }
          ...
          <div className="p-8">
            <form onSubmit={handleLogin} className="space-y-5">
              <div>
                ...
                <input id="email" autoComplete="email" type="email" required value={form.email} onChange={set("email")} placeholder="emergency@medilink.com"
