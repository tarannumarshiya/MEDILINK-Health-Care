...
export default function InsuranceLoginPage() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    const allowedSet = new Set(["INSURANCE_ADMIN", "INSURANCE", "HOSPITAL_ADMIN", "SUPER_ADMIN"]);
    if (!profile || !allowedSet.has(profile.role) || !profile.is_active) {
      await supabase.auth.signOut();
      setError(!allowedSet.has(profile?.role ?? "") ? "Access denied. Insurance staff only." : "Account inactive. Contact Super Admin.");
      setLoading(false);
      return;
    }
          ...
          <div className="p-8">
            <form onSubmit={handleLogin} className="space-y-5">
              <div>
                ...
                <input id="email" autoComplete="email" type="email" required value={form.email} onChange={set("email")} placeholder="insurance@medilink.com"
