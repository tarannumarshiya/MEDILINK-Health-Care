...
|---|---|---|---|
| **Public Portal** | `/` (Home Landing Page) | Hero banner, specialty departments, medical team, checkup packages, testimonials, interactive chatbot | Loading, Empty, Error, Responsive ✅ |
| **Public Portal** | `/about` | Hospital mission, clinical leadership, accreditations, facility photo galleries | Responsive, Static Content ✅ |
| **Public Portal** | `/services` | Directory of 24/7 medical services, specialized clinical programs | Responsive, Cards Grid ✅ |
| **Public Portal** | `/departments` | Department catalog with filterable search and appointment booking links | Search/Filter, Grid ✅ |
| **Public Portal** | `/doctors` | Clinician directory displaying qualifications, experience, and consultation fees (PII email stripped) | Search/Filter, Cards ✅ |
| **Public Portal** | `/packages` | Health checkup packages with itemized tests and pricing | Cards, Modal Booking ✅ |
| **Public Portal** | `/contact` | Public inquiry submission form with live validation and character limits | Form Validation, Success Banner ✅ |
| **Public Portal** | `/appointment` | Multi-step appointment booking form with department selection and date/time pickers | Date Validation, Confirmation Modal ✅ |
| **Public Portal** | `/patient/track` | Zero-leakage public appointment tracking by code (APT-XXXX) | Error 404, Minimal Public State ✅ |
| **Public Portal** | `/pharmacy` | E-pharmacy medicine catalog, search, category filter, slide-out Cart Drawer | Cart Context, Stock Checks, Drawer ✅ |
| **Public Portal** | `/pharmacy/track` | Public pharmacy order tracking by UUID or phone number | Order Status Timeline, Error 404 ✅ |
| **Public Portal** | `/medicine-remainders` | Public dosage reminder management form with schedule frequency options | Validation, Frequency Dropdown ✅ |
| **Public Portal** | `/emergency` | Public emergency SOS dispatch button and live hospital ward bed availability metrics | SOS Modal, Ward Occupancy Progress ✅ |
| **Patient Portal** | `/patient/login` & `/patient/register` | Patient email/password authentication and registration forms | Auth Error, Form Validation ✅ |
| **Patient Portal** | `/patient/journey` | Interactive patient visual journey timeline from booking to discharge | Dynamic Step Indicator ✅ |
| **Patient Portal** | `/patient/dashboard` | Tabbed patient portal (Appointments, Prescriptions, Lab Reports, Invoices, Reminders) | DataTables, PDF Viewers, Pay Modals ✅ |
| **Staff Auth** | `/login` & portal logins | Role-based staff authentication with automated redirect to assigned dashboard | Redirect Logic, Invalid Auth Alert ✅ |
| **Hospital Admin** | `/admin/dashboard` | Department CRUD, Doctor availability, Appointment approval/rejection, Contact inquiries | Action Modals, Tabbed Views ✅ |
| **Doctor Workstation**| `/doctor/queue` | Clinical consultation queue, start consultation, structured prescription writer, patient history | Queue Table, Prescription Form, History ✅ |
| **Laboratory** | `/lab/queue` | Diagnostic test queue, status stepper, report upload modal, pathologist verification | Status Badges, File Upload UI ✅ |
| **Pharmacy Queue** | `/pharmacy/queue` | In-hospital prescription dispensing queue, public order processing, inventory CRUD | Stock Alerts, Quantity Counters ✅ |
| **Reception Desk** | `/reception/dashboard` | Walk-in queue position management, fast-track check-in form, doctor availability roster | Live Queue Reordering, Check-in Modal ✅ |
| **Billing & Finance**| `/billing/dashboard` | Itemized invoice generator, payment recorder, live revenue graphs, transaction history | Revenue Charts, Bill Modal, QR Modal ✅ |
| **Health Insurance** | `/insurance/dashboard` | Policy register, claims review, adjudication modal (approve/reject with reason) | Claim Cards, Approval Modal ✅ |
| **Emergency Triage** | `/emergency/dashboard` | SOS dispatch queue, emergency case intake, triage severity flags, bed assignment grid | Bed Grid, Severity Badges, SOS Alert ✅ |
| **Telemedicine** | `/telemedicine/dashboard` | Remote video session schedule, session status controller, expired session flags | Session Cards, Link Launchers ✅ |
| **Super Admin** | `/super-admin/dashboard` | Staff account management, role assignment, account activation toggle, audit logs | User Management Table, Audit Viewer ✅ |
...
- ✅ Complete responsive layout and theme token system in `globals.css` supporting both OKLCH and legacy hex tokens.
- ✅ Shared `DashboardShell` layout with mobile drawer navigation, user profile display, and active role tags.
- ✅ Client-side idle timeout security system with visual countdown modal and automated logout.
- ✅ Multi-step online appointment booking form with instant validation and confirmation modals.
- ✅ E-pharmacy slide-out cart drawer with stock quantity limits and server-side price reconciliation.
- ✅ Standardized clinical and operational data tables (`DataTable`) with status badges and action triggers.
- ✅ Hospital bill inspection modal (`HospitalBill.tsx`) with itemized charge breakdowns and UPI QR code generator (`RazorpayQR.tsx`).
...
- ⏳ **Live Payment Gateway Gateway Modal:** In `PAYMENT_MODE=razorpay`, relies on Razorpay Checkout JavaScript SDK loaded via `RazorpayScript.tsx`; requires active Razorpay test keys in browser environment.
...
### 5.3 Testing & Validation Disclaimer
> [!NOTE]
